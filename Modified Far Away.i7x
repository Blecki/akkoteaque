Version 5 of Modified Far away by Anthony Casteel begins here.

Use authorial modesty.

"Creates an adjective for far-off objects which cannot be touched."

"based on Far Away by Jon Ingold"
...
A backdrop is usually distant.
A thing can be visible-at-a-distance. A thing is usually not visible-at-a-distance.
A backdrop is usually visible-at-a-distance.

Definition: a thing is far-off if it is distant or it is enclosed by something distant.
Definition: a thing is visible at a distance if it is not far-off or it is visible-at-a-distance.
...
Instead of examining a far-off thing (called the far-away-thing):
	If the noun is visible at a distance:
		Continue the action;
	Otherwise if we can't get at the far-away-thing:
		Consider the far-off message rules for the far-flung-thing;
	Otherwise:
		Continue the action.
...
[ This runs the rulebook. Means we can consider noun and second noun neatly in the accessibility rule above. ]
	...
	consider the distant objects rules for n;
	if rule failed:
		decide yes;
...
The last distant objects rule:
	rule fails.

Section - Messages

The far-off message rules are a thing based rulebook.

Far-off message rule when examining (this is the too far away to get a good look rule):
	say "[The far-flung-thing] [is-are] too far away to get a good look.", paragraph break;
	Stop the action.
	
The last far-off message rule (this is the default far-off message rule):
	say "[The far-flung-thing] [is-are] out of reach.", paragraph break.


Modified Far away ends here.

---- DOCUMENTATION ----

Additions and changes I have made -
	Things cannot be examined if they are too far away to get a good look.
	Things can be visible at a distance, which re-enables far away examining.
	The far-off messages are now a rule book, allowing the message to be easily customizable.
		-Comes with a simple default message and a message for examining far-off things.
