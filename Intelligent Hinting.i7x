Version 5 of Intelligent Hinting by Anthony Casteel begins here.
...
A puzzle can be puzzle-sequential. A puzzle is usually puzzle-sequential. A puzzle is usually privately-named. [The children of a sequential puzzle must be solved in the order they are defined in the source; the children of a not sequential puzzle can be solved in any order.]

A puzzle has a list of objects called the dependancies.
...
A task can be cart-required. A task is usually not cart-required.
A task can be lively-required, lively-nuetral, or ghostly-required. A task is usually lively-nuetral.
A task has a list of objects called the dependancies.
	...
	repeat with item running through tasks:
		...
		otherwise if item is unsolved and item is complete:
			...
			while tmpitem is not Winning-The-Game and tmpitem is not listed in list of cached-solved tasks:
				...
				let myctr be 0;
				If the requiree of tmpitem is not nothing:
...
When play begins (this is the setting up puzzle association tables rule):
	unless the number of not puzzle-sequential puzzles is 0:
...
To generate puzzle lookup tables for (problem - a puzzle):
	repeat with req running through unsolved tasks required by problem:
		repeat with step running through action-sequence of req:
			...
			If the step is the action of digging, associate the shovel with req in candidates; [Require the shovel for digging]
...
To decide what object is the best choice of the ranked puzzles under (problem - a puzzle):
	...
	If parent is not nothing:
		while parent is not nothing and parent is puzzle-sequential and parent is unsolved and parent is not problem and problem is not required by parent:
...
Suggesting is an action out of world applying to nothing.

Section - Suggest verbs - Not For Release

Understand "suggest" as suggesting.
...
Solving is an action applying to nothing.

Section - Solving Verbs - Not For Release

Understand "solve" as solving. 
...
To decide which stored action is the suggested action:
	...
	If a blocking scene is happening:
		Let BS be a random happening blocking scene;
		If puzzle-testing all, say "(Blocking scene: [BS])";
		Consider the blocking scene special action rules;
		If rule succeeded:
			If puzzle-testing all, say "(Scene action is: [relevant action])";
			Decide on relevant action;
	If puzzle-testing all, Say "(Macguffins: [macguffin-queue])";
	if the number of entries in macguffin-queue > 0 and steps must be taken to acquire entry ( number of entries in macguffin-queue ) of macguffin-queue, decide on relevant action;
	...
	if venue of puzz is not nothing and venue of puzz is not a room:
		...
		decide on the action of the person asked fake-actioning;
	If venue of puzz is not nothing:
		now relevant location is venue of puzz;
	[If the relevant location is the lighthouse apex and the player is in the elevator and the elevator is in the lighthouse apex:
		If puzzle-testing all, say "(Exit the elevator hack applies)";
		If puzz is cart-required:
			If the cart is in the elevator:
				Now relevant action is the action of the person asked pushing the cart out;
				Decide on relevant action;
		Otherwise:
			Now relevant action is the action of the person asked exiting;
			Decide on relevant action;]
	If puzz is cart-required:
		if puzzle-testing all, say "(We need the cart for this puzzle.)";
		If steps must be taken to acquire the cart:
			Decide on the relevant action;
		Otherwise:
			If location is not the relevant location:
				If puzzle-testing all, say "(Fetching cart)";
				If we can move to relevant location:
					If relevant action is the action of the person asked going down:
						If puzzle-testing all, say "(Lets try pushing the cart [relevant-direction])";
						Now relevant action is the action of the person asked pushing the cart to relevant-direction;
						if puzzle-testing all, say "(Created the action)";
					Otherwise:
						If relevant action is the action of the person asked entering the elevator:
							Now relevant action is the action of the person asked pushing the cart into the elevator;
							If puzzle-testing all, say "(Need to transport the cart on the elevator)";
						if puzzle-testing all, say "(Seems we decided on an action other than going: [relevant action])";
					Decide on relevant action;
	If puzz is lively-required:
		If puzzle-testing all, say "(The venue must be lively to perform this task.)";
		If the location of the projector is nothing:
			If puzzle-testing all, Say "(We need to go to the sitting room to activate the projector creating scene)";
			If we can move to the sitting room:
				Decide on relevant action;
		If steps must be taken to acquire the projector: 
			If puzzle-testing all, Say "(We need to fetch the projector){RD: [relevant-direction] AD: [actually-down]}";
			Decide on the relevant action;
		Otherwise:
			If the projector is switched off:
				Now relevant action is the action of the person asked switching on the projector;
				Decide on relevant action;
	If puzz is ghostly-required:
		If the location of the player is the relevant location:
			If the location of the player is lively:
	...
	if relevant location is not nothing and location is not venue of puzz:	
		...
		if we can move to relevant location:
			decide on relevant action;
	If the player is in the elevator: [Situations where we are actually riding the elevator are already handled]
		If puzzle-testing all, say "(We need to get out of the elevator)";
		If the cart is in the elevator:
			Now relevant action is the action of the person asked pushing the cart out;
		Otherwise:
			Now relevant action is the action of the person asked exiting;
	...
	if action-position of puzz + 1 is at most the number of entries in action-sequence of puzz :
		...
		if second noun part of candidate is a person (called partner) and partner is not visible:
			If candidate is not conversing:
				if puzzle-testing inferences, say "(trying to track down [partner])";
				if we can move to location of partner:
					decide on relevant action;
				now relevant action is the action of the person asked waiting;
...
To decide whether a move is implied by (act - a stored action):
	...
	If act is the action of the person asked digging:
		If steps must be taken to acquire the shovel, decide yes;
	decide no.
...
To decide which task is real next task in (problem - a task):
	Repeat with req running through dependancies of problem:
		If puzzle-testing all, say "(Checking problem dependancy [req])";
		if req is a task and req is unsolved:
			decide on real next task in req;
...
To decide which task is next task in (problem - a puzzle):
	Repeat with req running through dependancies of problem:
		...
		otherwise if req is a puzzle and req is unsolved:
			decide on next task in req;
	if problem is puzzle-sequential:
...
[				say "(**deciding on next task in [req]**)";]
	...
	otherwise: [problem is not sequential ]
		...
		unless saved endeavor is nothing: [problem is not sequential & something it requires is our saved endeavor.]
			If puzzle-testing all, say "(saved endeavor: [saved endeavor])";
			let tmptask be saved endeavor;
			unless problem is tmptask:
				while (requiree of tmptask is not nothing) and (tmptask is not required by problem):
					now tmptask is requiree of tmptask;
			If puzzle-testing all, say "(**[tmptask]**)";
			if tmptask is a task:
[				say "**tmptask deciding on [tmptask]**";]
				decide on real next task in tmptask;
...
[		say "**made it this far**";]
		...
		if candidate is a task and candidate is unsolved:
			...
			decide on real next task in candidate;
...
Printing suggestion is a truth state that varies.

To process appropriate action, actually performing and/or as a command: 
	If puzzle-testing all, say "[relevant action]";
	if as a command:
		If puzzle-testing all, say "(as a command)";
	...
	otherwise:
		...
		if relevant action is the action of the person asked going down:
			If puzzle-testing all, say "(Action is going down. [relevant-direction] [actually-down])";
...
action-suggesting		"[link]Try [relevant action][as]solve[end link].[line break]"
...
To decide whether shut (parent - a thing) causes surmountable difficulties: 
	...
	if parent is saved parent: [We've recursed back to here, possibly because the key to a locked thing in our way is on the other side of the locked thing; so give up entirely.]
		now relevant action is the action of the person asked fake-actioning;
		If puzzle-testing all, say "(Recursion A)";
		decide yes;
	...
	if parent is locked and parent provides the property matching key and matching key of parent is on-stage:
		...
		if steps must be taken to acquire matching key of parent:
			if matching key of parent is listed in macguffin-queue: [another recursion check, which catches circular loops of lock/key dependencies.]
				now relevant action is the action of the person asked fake-actioning;
				If puzzle-testing all, say "(Recursion B)";
				decide yes;
			...
			If puzzle-testing all, Say "(Demonstrate lockedness)";
			decide yes;
	If puzzle-testing all, Say "(There is no way around this obstacle)";
	decide no.
	...
	if macguffin is portable and macguffin is not part of something:
		if macguffin is touchable:
			if the number of things held by player < carrying capacity of player:
				If macguffin is a heavy thing:
					If macguffin is not the spare bulb: [Okay, this catches the one time taking a heavy thing is required]
						Decide no;
				If macguffin is the eagles nest:
					Remove macguffin from macguffin-queue, if present;
					Decide no;
				If macguffin is the crystal glass:
					Remove macguffin from macguffin-queue, if present;
					Decide no;
...
To decide whether we can move to (area - a room):
	...
	If the player encloses a heavy thing:
		Let H be a random heavy thing enclosed by the player;
		If puzzle-testing all, say "(Heavy thing [H] prevents movement)";
		Now relevant action is the action of the person asked dropping H;
		decide yes;
	[If area is the balcony room: [Accessing the balcony room requires climbing the ladder in the garden]
		If the balcony room door is locked and the room key is enclosed by the balcony room: [The default logic will give up, thinking there is no way to get to the key]
			If steps must be taken to acquire the ladder:
				If puzzle-testing all, say "(Accessing balcony room requires ladder)";
				Decide yes;
			Otherwise:
				If the location of the player is the garden:
					Now the relevant action is the action of the person asked climbing the ladder;
					Decide yes;
				If we can move to the garden:
					Decide yes;]
	If area is ladder-accessible:
		Consider the ladder accessibility rules for the area;
		If the rule succeeded:
			If puzzle-testing all, say "(We need the ladder to access [the area])";
			If the ladder encloses the player:
				Now the relevant action is the action of the person asked getting off the ladder;
				Decide yes;
			If steps must be taken to acquire the ladder:
				Decide yes;
			Otherwise:				
				If the location of the player is the ladder accessibility point of the area:
					Now the relevant action is the action of the person asked climbing the ladder;
					Decide yes;
				If we can move to the ladder accessibility point of the area:
					If the player does not enclose the ladder:
						Now relevant action is the action of the person asked taking the ladder;
					Decide yes;
	If (area is the bay room or area is the upstairs hallway) and (the player is not in the sitting room and the player is not in the upstairs hallway):
		If puzzle-testing all, say "(Short-circuit pathfinding for [area]: Targetting the sitting room instead)";
		Now area is the sitting room;
	If area is in the excavation:
		If the player is not in the excavation:
			If puzzle-testing all, say "([area] is in the excavation. Need livelyness to proceed)";
			If steps must be taken to acquire the projector:
				If puzzle-testing all, say "(Aquiring projector)";
				Decide yes;
			If the location of the player is the cellar:
				If the cellar is ghostly:
					Now the relevant action is the action of the person asked switching on the projector;
					Decide yes;
			Otherwise:
				If we can move to the cellar:
					Decide yes;
	If (the area is not in the excavation) and (the player is in the excavation):
		If puzzle-testing all, say "(We need livelyness to exit the excavation)";
		If steps must be taken to acquire the projector:
			Decide yes;
		If the location of the player is the mine:
			If the mine is ghostly:
				Now relevant action is the action of the person asked switching on the projector;
				Decide yes;
		Otherwise:
			If we can move to the mine:
				Decide yes;
	If (the area is in the high-sky) and (the player is not in the high-sky):
		If puzzle-testing all, say "(We need to ride the elevator)";
		If the player is enclosed by the elevator:
			Now relevant action is the action of the person asked pushing the green button;
			Decide yes;
		Otherwise if the location of the player is the machinery room:
			If the elevator is closed:
				Now saved parent is nothing;
				If shut elevator causes surmountable difficulties:
					Decide yes;
			Now relevant action is the action of the person asked entering the elevator;
			Decide yes;
		If we can move to the machinery room:
			Decide yes;
	If (the area is not in the high-sky) and (the player is in the high-sky):
		If puzzle-testing all, say "(We need to ride the elevator)";
		If the player is enclosed by the elevator:
			Now relevant action is the action of the person asked pushing the green button;
			Decide yes;
		Otherwise if the location of the player is the lighthouse apex:
			If the elevator is closed:
				Now saved parent is nothing;
				If shut elevator causes surmountable difficulties:
					Decide yes;
			Now relevant action is the action of the person asked entering the elevator;
			Decide yes;
		If we can move to the lighthouse apex:
			If puzzle-testing all, say "{RD: [relevant-direction] AD: [actually-down]}";
			Decide yes;
	...
	if way is a direction:
		...
		[if holder of player is not a room: [Stop the solver exiting, since it's an implicit action]
			now relevant action is the action of the person asked exiting;
			decide yes;]
		repeat with __portal running through visible closed doors: [Changed portal to __portal to avoid conflict with portal object]
			if the other side of __portal is the room way from the location:
				if puzzle-testing all, say "([the __portal] is in our way)";
				if __portal is not openable, decide no;
				if shut __portal causes surmountable difficulties, decide yes;
				[now relevant action is the action of the person asked opening __portal;
				decide yes;] [Again: Stop the solver from opening doors; it's implicit.]
		...
		Otherwise:
			now actually-down is false;
		If puzzle-testing all, say "{C RD: [relevant-direction] AD: [actually-down]}";
		decide yes;
	decide no.
...
To show puzzle tree for (problem - a puzzle) with indent (indent - a number):
	say indent spaces;
	say "[bold type][problem][roman type] [if problem is solved](solved) [end if][if problem is not puzzle-sequential](not sequential)[end if][if problem is saved endeavor]***[end if][line break]";
	repeat with puzz running through things required by problem:
		if puzz is a puzzle:
			show puzzle tree for puzz with indent ( indent + 4 ) ;
		otherwise:
			say ( indent + 4 ) spaces;
			say "[puzz] [if puzz is complete](complete)[end if][if puzz is cart-required](cart needed)[end if][if puzz is lively-required](livelyness needed)[end if][if puzz is ghostly-required](ghostlyness needed)[end if] [if puzz is saved endeavor]***[end if][line break]";
		If the number of entries in dependancies of puzz is not 0:
			Say ( indent + 5 ) spaces;
			Say "Also dependant on: [dependancies of puzz][line break]".
